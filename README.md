...
- Выдаём по SPI данные для сдвиговых регистров. Для одной матрицы 32x16 это 16 байт (16 8-битных регистров).
- Устанавливаем лог. 0 на ножке nOE.
- Устанавливаем лог. уровни на ножках A и B в соответствии с обновляемой группой светодиодов (одной из четырёх). Это подаёт +5В на аноды светодиодов выбранной группы.
- Выдаём на ножку SCLK короткий положительный импульс. Это подаёт землю на катоды светодиодов в соответствии с загруженными в регистры байтами.
- Устанавливаем лог. 1 на ножке nOE. При этом четверть экрана (одна группа светодиодов) загорается и горит до следующего обновления следующей группы светодиодов.

Повторяем пункты 1-5 с постоянным периодом.

A, B являются селекторами строк, эти выводы представляют собой 2-битное двоичное число, позволяющее выбрать одну из четырех строк в последовательности. Выбор ряда означает, что светодиоды в выбранном ряду питают свои анодные контакты. Всего имеется 16 рядов (A1-A16), и они объединены в группу по 4. Каждая группа рядов представлена одним цветом на изображениях ниже. Посмотрите на таблицу: включение A и B с низким сигналом означает выбор строк, помеченных A1, A5, A9 и A13.

![image](https://github.com/GreyRaccoon159/Flappy_Bird/assets/152299663/4a3afc5d-f9ce-42f8-8c7f-4b4bb045a982)

# Подключение P10 к STM32

В рамках лабораторной работы соединение с матрицей осуществляется путем подсоединения проводов сигнала от платы STM32 к специальному кабелю, установленному в крайнем левом разъеме (На плате подписан как INPUT).

Крайний правый порт предназначен для последовательного соединения нескольких панелей (На плате подписан как OUTPUT).

Ниже представлен пример расположения контактов порта.
...
- **PA4** - *OE* - отвечает за включение / выключение панели
...
- **PA0** - *BUTTON* -канал с АЦП для снятия значения с реостата

# Переменные проекта

| Наименование | Назначение | Тип данных | Диапазон измерений |
|--------------|------------|------------|--------------------|
| xs           | Координата по оси x, от которой производится отрисовка птички | uint16_t | 0~65535 |
| ys           | Координата по оси y, от которой производится отрисовка птички | float | - |
| xh, yh, xh1, yh1, yl, yl1 | Координаты по осям oX и oY, от которых рисуется препятствие | float | - |
| prop_count   | Счётчик препятствий, при каждой итерации появляется новое препятствие | uint8_t | 0~255 |
| jump_flag    | Логическая метка прыжка при нажатии на кнопку | uint8_t | 0~255 |
| flag         | Логическая переменная с таймера | uint8_t | 0~255 |
| pbutton      | Предыдущее нажатие кнопки | int | -32768~32767 |
| button       | Текущее нажатие кнопки | int | -32768~32767 |
| Pisdead      | Игрок метр, отвечает за запуск/остановку игрового цикла | uint16_t | 0~65535 |
